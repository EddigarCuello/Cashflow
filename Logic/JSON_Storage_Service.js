...
export const JSONStorageService = {
  ...
  // Función para guardar un préstamo
  saveLoan: async (cedula, loanData) => {
    try {
      const data = await JSONStorageService.loadData();
      ...
      if (userIndex >= 0) {
        // Agregar el préstamo al array de préstamos del usuario
        if (!data.users[userIndex].prestamos) {
          data.users[userIndex].prestamos = [];
        }
        
        // Agregar fecha de creación y estado por_aprobar al préstamo
        const loanWithDate = {
          ...loanData,
        ...
        data.users[userIndex].prestamos.push(loanWithDate);
        return await JSONStorageService.saveData(data);
      }
  ...
  // Función para obtener todos los préstamos de usuarios no admin
  getNonAdminLoans: async () => {
    try {
      const data = await JSONStorageService.loadData();
      return data.users
        .filter(user => !user.isAdmin)
        .flatMap(user => 
          (user.prestamos || []).map(loan => ({
            ...loan,
